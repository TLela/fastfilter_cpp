#ifndef THREEWISE_XOR_BINARY_FUSE_FILTER_XOR_FILTER_LOWMEM_ADAPTED_H_
#define THREEWISE_XOR_BINARY_FUSE_FILTER_XOR_FILTER_LOWMEM_ADAPTED_H_
 ...
 */
namespace xorbinaryfusefilter_lowmem_adapted {
...
public:
  ...
  explicit XorBinaryFuseFilter(const size_t size) {
    ...
    //Implement suggestion in paper to choose 3 segments for small sizes
    if (size <= 10000){
      this->segmentLength = ceil(capacity/3.0);
      this->segmentCount = 3;
...
Status XorBinaryFuseFilter<ItemType, FingerprintType, HashFamily>::AddAll(
  ...
  while (true) {
    ...
    for (size_t i = 0; i < size; i++) {
      ...
      for (int hi = 0; hi < 3; hi++) {
        ...
        //printf("Current parameters: %llu, %u, %llu\n", hash, hi, t2hash[index]);
      }
    }
    delete[] startPos;
    printf("Check Count Mask: %u\n", countMask);
    if (countMask >= 0x80) {
  ...
  printf("Filter contains fingerprints: %hu\n", fingerprints[0]);
  printf("Add all succesfull\n");
  delete[] reverseOrder;
  delete[] reverseH;
